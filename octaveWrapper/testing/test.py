...
controlPts_oct=numpy.zeros([noPtsX,4,noPtsY])

for i in range(noPtsX):
    controlPts_oct[i]=numpy.transpose(controlPts[i*noPtsX:(i+1)*noPtsX])
    pass
# controlPts_oct[1]=numpy.transpose(controlPts[noPtsX:])
# print(controlPts_oct[0])
...
# controlPts_oct=numpy.reshape(controlPts_oct,[2,4,2])
octave.refinePatch(numpy.transpose(controlPts_oct),noPtsX,noPtsY,uKnot,vKnot,refineLevel,refU,refV)
shell = sio.loadmat('shell_refined.mat')
P = int(shell['p'])
Q = int(shell['q'])
uKnot = shell['uKnot'][0]
vKnot = shell['vKnot'][0]
...
# Setting new control points for surface
# Set surface degrees
surf_L.degree_u = Q
surf_L.degree_v = P
...
refU=1
refV=1
p, q, uKnot, vKnot, noPtsX, noPtsY, weights, controlPts = octave.makeCantileverShell(a,b,refineLevel,refU,refV,nout=8)
# out = octave.makeCantileverShell()
