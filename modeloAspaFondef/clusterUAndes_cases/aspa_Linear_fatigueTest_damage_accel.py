...
def fatigueAnalysis(i_cycle,t_Final,t_steadyState):
    ...
    lastNode = i_output[-1]
    ops.recorder('Node', '-file', f'Linear_fatigue_{i_cycle}.out', '-closeOnWrite', '-time', '-node', *[lastNode], '-dof', *[2], *['accel'])
...
def writeOnFile(string):
    # Writing to file
    with open("modalResults_linear.txt","a") as file:
...
file_modal_Results = open("modalResults_linear.txt",'a')
...
while nCycles < nCycles_max:
    ...
    while nCycles < nCycles_this:
        ...
        ops.recorder("mpco",f"aspa_linear_{iTest}",
            "-N","displacement",
