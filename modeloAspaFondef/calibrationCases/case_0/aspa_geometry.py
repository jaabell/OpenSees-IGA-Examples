...
# Input parameters
refine_U = True
uCuts = np.array([0.011+0.094169, 0.251+0.0124, 0.696-0.0043, 1.141-0.00561, 1.586])  # Places to define sections
# uCuts = np.array([0.011, 0.251])  # Places to define sections
# uCuts = np.array([0.009])  # Places to define sections
# uCuts = np.array([])  # No transverse bending strips
knotInsert_first = 0.01
...
for i in range(len(uCuts)):
    ...
    print('zLimits = ', zLimits)
    print('zLength = ', zLength)
    print('')

    parametricInt = interpolate.interp1d(zLimits, [0.0, 1.0])
    ...
    boundingBox = (np.array(surf_top.bbox)+np.array(surf_bottom.bbox))/2
    boundingBox = np.array(surf_top.bbox)


    print("boundingBox = ", boundingBox)

    boundingBox = np.array(cutSurfaces_top[1].bbox)
    print("boundingBox = ", boundingBox)
...
if len(uCuts) == 0:
    # Now adding surfaces to the global surfaces dictionary in case of no transverse bending strips

    i = 0

    surfaces=[topSurf_original, bottomSurf_original]
    surfList.append(topSurf_original)
    surfList.append(bottomSurf_original)
...
for i in range(int(len(surfList)/2)):
    ...
    surfList_vis.append(operations.translate(
        surfList[2*i], (0, -1e-2*2, i/10), inplace=False))
    surfList_vis.append(operations.translate(
        surfList[2*i+1], (0, 1e-2*2, i/10), inplace=False))

    offsets.append(i/10)

# offsets = np.cumsum(np.array(offsets))
# offsets += offsets[1]
...
for i in range(len(bendingStripMap_U)):
    ...
    # print(f"bendingStrip_{bendingStripTag}")
    ...
    if i == 0 or i % 2 == 0:
        surfList_vis.append(operations.translate(
            bendingStrip, (0, 0, 1*(i/2)/10), inplace=False))
    else:
        surfList_vis.append(operations.translate(
            bendingStrip, (0, 0, 1*(i-1)/2/10), inplace=False))
...
if len(uCuts)!=0:
    for i in range(len(bendingStripMap_V)):
        ...
        if i == 0 or i % 2 == 0:
            ...
            surfList_vis.append(operations.translate(
                bendingStrip, (0, -2e-2*2, zOffset), inplace=False))
            print(zOffset)
        else:
            zOffset = 0
            zOffset = ((i)/2)/20
...
# Render the aspa
colors = ['midnightblue', 'midnightblue', 'darkgoldenrod', 'darkgoldenrod', 'green', 'green', 'maroon','maroon', 'darkslateblue', 'darkslateblue', 'darkcyan', 'darkcyan']
bSs = ['grey']*22
colors.extend(bSs)

container.render(evalcolor = colors)
...
print('bendingStrip_dict = ', bendingStrip_dict)
