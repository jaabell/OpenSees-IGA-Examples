...
def rayleighDamping():
    ...
    wi = ws[0]**0.5; zetai = 0.8/100 # 5% in mode 1
    wj = ws[2]**0.5; zetaj = 1.0/100 # 2% in mode 6
...
def eigen(nModes, plot_modes = False):
    ...
    modes_flapwise = [1,3,4] 

    modes_matrix = np.zeros([10,len(modes_flapwise)])
    nMode_exp = 0
    ...
    if plot_modes:
        ...
        for nMode in modes_flapwise:
            ...
            mode_print = factor*np.array([ops.nodeEigenvector(n,nMode,2) for n in i_output ])/abs([ops.nodeEigenvector(n,nMode,2) for n in i_output ][-1])
            modes_matrix[:,nMode_exp] = mode_print
            nMode_exp += 1
...
def writeOnFile(string):
    # Writing to file
    with open("modalResults_3.txt","a") as file:
...
file_modal_Results = open("modalResults_3.txt",'a')
