...
for case in cases:
  ...
  # Simple model
  fileNames = sorted(list(glob.glob(case + '/aspa_linearSimple_*.mpco')))  # Simple model, just to check the damage distribution is continous
  shellName = case + '/matData/shellSimple'
  ...
  stages_arr = np.arange(3,41+2,2)

  # stages_arr = [3]

  print("stages_arr = ", stages_arr)

  print(len(stages_arr))

  currStage = -1

  for (file,stage) in zip(fileNames,stages_arr):

      file = h5py.File(file, 'r')

      stagesData = file
      stagesData_keys = stagesData.keys()
      numStages = len(stagesData_keys) - 2

      print('numStages = ', numStages)

      stages = np.arange(3,2*numStages+2,2)
      ...
      for stage in stages:
          ...
          for time in range(nTimes):
            ...
            for (surf,i) in zip(surfList,range(len(surfList))):
              ...
              if i == 3:
                i_dam +=1
                plt.plot(i_dam,damage[-1,-6,7],'-or')

              
              


              matName = f'{shellName}_{i}_{time+currStage}.mat'
