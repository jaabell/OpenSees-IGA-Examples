...
ops.fixZ(-0.229, 1, 0, 1)  # First row
...
# nSpins = 10
ω_max = 4 * 6.28  # Hz to rad/s
t_steady = 5 # s
amplitude = 0.14 / 2 # 14 cm de carrito
# tMax = (nSpins*2*np.pi/ω_max) # seconds
tMax = 10 # seconds
# deltaT = 0.005
deltaT = 0.01
# t = np.arange(0, tMax, deltaT) ** 1/2
t = (np.linspace(0, 1, int(tMax/deltaT)) ** 1/2) * tMax
# t = (np.linspace(0, 1, int(tMax/deltaT)) ) * tMax
# t = (np.linspace(0, 1, int(tMax/deltaT)) ** 2) * tMax
# t = np.arange(0, tMax, deltaT) ** 2

# dispY = amplitude * np.sin( ω*t )
...
ops.timeSeries("Path", 1, '-time', *(t.tolist()), '-values', *(dispY.tolist()))
...
nodesOnBase = np.concatenate([surface_0_nodes, surface_1_nodes])
...
for j in range(nSteps):
    ...
    if (ops.analyze(1, deltaT) != 0):
        ...
        fileName_stl = f'figures/surf_{j}.stl'
