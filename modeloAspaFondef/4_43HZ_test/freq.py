...
def generateOscillation(amplitude,t_steady,tMax,ω_min, ω_max,nPoints_accel,nPoints_steady):
	...
	t=np.arange(0,t_steady+deltaT,deltaT)
	...
	t2=np.arange(t_steady+deltaT,tMax+2*deltaT,deltaT)
	y2=np.sin(2*np.pi*ω_max*t2)
	# t2-=deltaT
	...
	t_int2=np.linspace(t_steady+deltaT,tMax,(tMax-t_steady)*nPoints_steady)



	t_int=np.concatenate([t_int1,t_int2[1:]])
	...
	deltaT_int = interp1d(t_int,deltaT_vector)

	return [deltaT_int,t_int,y_int]
...
# nPoints_accel=400
# nPoints_steady=400
# [deltaT_int,t,y] = generateOscillation(amplitude,t_steady,tMax,ω_min, ω_max,nPoints_accel,nPoints_steady)
# dispYpp=np.gradient(y)
# plot(t,dispYpp, '-r')
# plot(t,y,'-b')
# print(len(t))
