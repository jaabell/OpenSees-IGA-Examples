...
for surface_name in surfaces_data_loaded:
    ...
    surface['elementTags'] = np.arange(patchTag+1, lastElTag+1 ,1) # inclying lastElTag
...
def gravityAnalysis(nSteps,firstTime=True):
...
def rayleighDamping():
    ...
    wi = ws[0]**0.5; zetai = 2.9941/100.0 # 5% in mode 1
    wj = ws[1]**0.5; zetaj = 1.3511/100.0 # 2% in mode 6
...
def resetAnalysisAndAddPathSp(firstTime = True):
...
def fatigueAnalysis(i_cycle,t_Final,t_steadyState):
    ...
    while current_time<t_Final:
        ...
        fileName_stl = f'/home/rodrigo/FelipeElgueta/OpenSees-IGA-Examples/modeloAspaFondef/4_43HZ/figuresLinear_fatigue_{i_cycle}/surf_{surfName}.stl'
        surfName+=1
...
def createFolder(i_cycle):
    ...
    parent_dir = '/home/rodrigo/FelipeElgueta/OpenSees-IGA-Examples/modeloAspaFondef/4_43HZ/'
...
while nCycles < nCycles_max:
    ...
    # Resetting model and setting equalDofs again

    # for node in ops.getNodeTags():
    #     node=int(node)
    #     ops.setNodeDisp(node,1,0.0,'-commit')
    #     ops.setNodeDisp(node,2,0.0,'-commit')
    #     ops.setNodeDisp(node,3,0.0,'-commit')

    #     ops.setNodeVel(node,1,0.0,'-commit')
    #     ops.setNodeVel(node,2,0.0,'-commit')
    #     ops.setNodeVel(node,3,0.0,'-commit')

    #     ops.setNodeAccel(node,1,0.0,'-commit')
    #     ops.setNodeAccel(node,2,0.0,'-commit')
    #     ops.setNodeAccel(node,3,0.0,'-commit')

    ops.reset()
    # setEqualDofsAndBase()

    # Setting up next step
    gravityAnalysis(10,firstTime=False)
