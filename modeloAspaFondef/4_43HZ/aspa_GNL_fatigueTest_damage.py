...
def fatigueAnalysis(i_cycle,t_Final,t_steadyState):
    ...
    ops.recorder('Node', '-file', f'GNL_fatigue_{i_cycle}.out', '-closeOnWrite', '-time', '-node', *[3033], '-dof', *[2], *['disp'])
    ops.recorder('Node', '-file', f'GNL_fatigue_tip_{i_cycle}.out', '-closeOnWrite', '-time', '-node', *[2320], '-dof', *[2], *['disp'])
    ...
    while current_time<t_Final:
        ...
        fileName_stl = f'/home/rodrigo/FelipeElgueta/OpenSees-IGA-Examples/modeloAspaFondef/4_43HZ/figuresGNL_fatigue_{i_cycle}/surf_{surfName}.stl'
        surfName+=1
...
def createFolder(i_cycle):
    ...
    directory = f'figuresGNL_fatigue_{i_cycle}'
...
def writeOnFile(string):
    # Writing to file
    with open("modalResults.txt","a") as file:
        # Writing data to a file
        file.write(string_result)


file_modal_Results = open("modalResults.txt",'a')
nCycles_max = 1e9
nCycles = 0
i_cycle = 0
nModes = 6
