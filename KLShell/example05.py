...
controlPts = np.zeros((4,3,3,3));

# first zKnot 

x1 = np.sqrt(R*R     + R*R*tan(deg2rad*20)**2);
x2 = np.sqrt(Rii*Rii + Rii*Rii*tan(deg2rad*20)**2);
x3 = np.sqrt(Ri*Ri   + Ri*Ri*tan(deg2rad*20)**2);

controlPts[0:2,0,0,0] = [R*cos(deg2rad*(90-phi)), R*sin(deg2rad*(90-phi)), 0, 1];
controlPts[0:2,1,0,0] = [x1*cos(deg2rad*(70)), x1*sin(deg2rad*(70)), 0, 1];
controlPts[0:2,2,0,0] = [0, R, 0, 1];
