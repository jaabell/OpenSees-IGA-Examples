...
Pz = 1e0 / len(firstRigidLine)
P_here=0
totalLoad=0
for n in firstRigidLine:
    nodeToLoad = int(n + noPtsX - 1)
    if nodeToLoad == firstRigidLine[0] + noPtsX - 1:
        P_here=Pz/2
        ops.load(nodeToLoad, 0.0, 0.0, P_here)
        totalLoad+=Pz/2
    elif nodeToLoad == firstRigidLine[-1] + noPtsX - 1:
        P_here=Pz/2
        ops.load(nodeToLoad, 0.0, 0.0, P_here)
        totalLoad+=Pz/2
    else:
        P_here=Pz+Pz/(len(firstRigidLine)-2)
        ops.load(nodeToLoad, 0.0, 0.0, P_here)
        totalLoad+=P_here
...
for n in firstRigidLine:
    nodeToLoad = int(n + noPtsX - 1)
    print("nodeToLoad: ", nodeToLoad)
    print("ops.nodeDisp(nodeToLoad): ", 1000*(ops.nodeDisp(nodeToLoad)[3]), "mm")
