...
surf1=operations.rotate(surf,90,axis=2)
operations.translate(surf1,[-4.953,4.953,0],inplace=True)
surf2=operations.translate(surf1,[0,0,-10.35/2])
surf3=operations.rotate(surf2,90,axis=2)


mSurf.add([surf,surf1,surf2,surf3])
# surfVisualize(mSurf,hold=True)


mSurf.sample_size=10

for surface in mSurf:
    surface.evaluate()

from geomdl.visualization import VisMPL

# Visualization configuration
mSurf.vis=VisMPL.VisSurface(ctrlpts=True, legend=False)
...
for j in range(nSteps):
    ...
    if result != 0:
        ...
        data[j + 1, 0] = abs(ops.nodeDisp(1, 1)) # Point B
        data[j + 1, 0] = abs(ops.nodeDisp(132, 1))
