...
surfL.set_ctrlpts(controlPts.tolist(), 2, 12)
...
interfacePoints, nodesOnLeftPatch=edgeGetter(surfL,"01","11")

nodesOnRightPatch = edgeGetter(surfR,"00","10")[1]


bendingStrip=makeBendingStrip(nodesOnLeftPatch,interfacePoints,nodesOnRightPatch, direction = 'v')
...
for n in [23, 24]:
    ...
    print(ops.nodeCoord(n))
    print(ops.nodeCoord(n+2))

    ops.equalDOF(n, n + 2, 1, 2, 3)


# nodesOnPatches = [11, 12, 25, 23, 24, 38]
nodesOnPatches = [21, 22, 23, 24, 27, 28]
...
for i in range(len(nodesOnPatches)):
    ...
    print(ops.nodeCoord(const))
    print(ops.nodeCoord(ret))
...
for j in range(nSteps):
    ...
    if result != 0:
        break
        exit(-1)
    data[j + 1, 0] = abs(ops.nodeDisp(48, 3))
    data[j + 1, 1] = abs(ops.nodeDisp(48, 1))
